...
ui <- navbarPage(
  ...
  header = tags$head(tags$script(HTML("
    ...
    $(document).on('shiny:outputinvalidated', function(e) {
      ...
      if (e.target.id === 'cmp_vis_w') showSpinner('cmp_spinner_w');
      if (e.target.id === 'cmp_vis_m') showSpinner('cmp_spinner_m');
    });
    $(document).on('shiny:value', function(e) {
      if (e.target.id === 'net_vis')   hideSpinner('net_spinner');
      if (e.target.id === 'cmp_vis_w') hideSpinner('cmp_spinner_w');
      if (e.target.id === 'cmp_vis_m') hideSpinner('cmp_spinner_m');
    });
  "))),
  ...
  tabPanel("Sex Comparison",
    sidebarLayout(
      ...
      mainPanel(width=9,
        h3("Comorbidity networks by sex", align="center"),
        fluidRow(
          column(6, h4("Women", align="center"),
                    div(id="cmp_spinner_w",
                      style="text-align:center;padding:30px;font-size:16px;color:#888;display:none;",
                      HTML("&#9696; Loading&hellip;")),
                    visNetworkOutput("cmp_vis_w", height="520px")),
          column(6, h4("Men",   align="center"),
                    div(id="cmp_spinner_m",
                      style="text-align:center;padding:30px;font-size:16px;color:#888;display:none;",
                      HTML("&#9696; Loading&hellip;")),
                    visNetworkOutput("cmp_vis_m", height="520px"))
        ),
        hr(),
...
server <- function(input, output, session) {
  ...
  ## helper: build ego-network visNetwork objects for one sex
  make_cmp_net <- function(sex, code, rr_min, n_min, dir_only, show_interactions) {
    dt <- load_network(sex, input$cmp_win)
    if (is.null(dt)) return(NULL)
    ## always filter by RR/N/dir
    dt_flt <- dt[RR_shrunk >= rr_min & cases_event >= n_min]
    if (dir_only) dt_flt <- dt_flt[dir_sig==TRUE]

    ## 1st-degree edges: directly involving the selected disease
    dt1 <- dt_flt[disease_a==code | disease_b==code]
    if (!nrow(dt1)) return(NULL)

    ## neighbours (1st degree)
    neighbour_codes <- unique(c(
      dt1[disease_a==code, disease_b],
      dt1[disease_b==code, disease_a]
    ))

    ## optionally add edges between neighbours (2nd-degree)
    if (show_interactions && length(neighbour_codes) > 1) {
      dt2 <- dt_flt[disease_a %in% neighbour_codes & disease_b %in% neighbour_codes]
      dt_use <- unique(rbindlist(list(dt1, dt2), fill=TRUE))
    } else {
      dt_use <- dt1
    }

    ## build node table
    all_codes <- unique(c(dt_use$disease_a, dt_use$disease_b))
    node_info <- unique(rbindlist(list(
      dt_use[, .(id=disease_a, label=disease_a,
                 title=paste0("<b>",disease_a,"</b> – ",name_a,"<br>",category_a),
                 color=color_a)],
      dt_use[, .(id=disease_b, label=disease_b,
                 title=paste0("<b>",disease_b,"</b> – ",name_b,"<br>",category_b),
                 color=color_b)]
    )), by="id")
    ...
    edges <- dt_use[, .(
      ...
      value=pmin(RR_shrunk,20),
      title=paste0("RR: ",round(RR_shrunk,2),
                   " [",round(CI_low_RR_shrunk,2),"–",round(CI_high_RR_shrunk,2),"]",
                   "<br>N: ",cases_event,
                   "<br>θ: ",round(theta,3),
                   "<br>Dir.sig: ",dir_sig),
      color=ifelse(dir_sig,"#E06C00","#AAAAAA"),
  ...
  output$cmp_vis_w <- renderVisNetwork({
    make_cmp_net("women", cmp_code(), input$cmp_rr, input$cmp_n, input$cmp_dir, input$cmp_interactions)
  })
  output$cmp_vis_m <- renderVisNetwork({
    make_cmp_net("men",   cmp_code(), input$cmp_rr, input$cmp_n, input$cmp_dir, input$cmp_interactions)
  })
