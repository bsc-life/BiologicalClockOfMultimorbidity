...
ui <- navbarPage(
  ...
  tabPanel("Disease Explorer",
    sidebarLayout(
      ...
      mainPanel(width=9,
        h3("Comorbidities of the selected index disease", align="center"),
...
server <- function(input, output, session) {
  ...
  output$sx_plot <- renderPlotly({
    ...
    if (ptype == "volcano") {
      ...
      dt <- dt[order(DiffMean)]
      dt[, Code := factor(Code, levels=Code)]
      p <- ggplot(dt,
            aes(x=Code, y=DiffMean, fill=color,
        ...
        labs(x=NULL, y="Mean age difference  (Women − Men, years)") +
  ...
  output$sx_table <- renderDT({
    ...
    out <- if (ptype=="volcano") {
      ...
      age_dt()[, .(Code, Name=disease_name, Category=catename,
                   ...
                   `Mean age M`=round(MenMean,1), FDR=signif(FDR,3))]
    } else {
